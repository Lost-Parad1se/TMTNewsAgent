...
- Web Access Layer：支持 static / Jina / 可选本地 CDP / 本地浏览器历史检索骨架。
...
TMTNewsAgent/
  ...
  src/
    ...
    web_access/           # static / Jina / optional CDP / browser history
...
```bash
python -m src.cli --mode manual_url --topic "游戏出海动态" --urls "https://mp.weixin.qq.com/xxx" "https://mp.weixin.qq.com/yyy" --web-access-strategy auto
```
...
前端支持输入公司/关键词、时间窗口、手动 URL、授权搜索或本地浏览器历史匹配，并直接生成 Markdown 报告预览和 `.md` 下载文件。若要搜索 3 天内的微信公众号文章，可选择“授权搜索”，推荐配置 `TAVILY_API_KEY`，系统会聚焦查询 `site:mp.weixin.qq.com/s`；未配置授权搜索 API 时，请使用手动 URL、本地历史或 CSV/手动正文模式。

前端也可以切换基座模型 provider：`mock`、`gemini`、`qwen`、`deepseek`、`glm`、`openai-compatible`。API Key 仍然只从 `.env` 读取，前端不会要求输入或传输密钥。
...
Phase 13 新增 `src/web_access/`，用于在 collector 和正文清洗之间提供更稳健、可选、低侵入的信息获取层。设计参考 eze-is/web-access 的策略选择、site patterns、本地浏览器辅助和历史检索思路，但本项目只实现合规公开信息读取，不做绕过登录、验证码、反爬或平台访问限制。

默认策略：

- 普通文章页：优先 `jina`，再回退 `static`。
- 微信公众号 URL：优先 `static`；如果显式 `--enable-cdp`，才尝试本地 CDP；否则进入 `manual_required`。
- PDF URL：使用 `static`。
- 关键词搜索仍由 `SearchCollector` 处理，URL 抽取后再进入 Web Access Layer。

可选能力：

- `--web-access-strategy auto/static/jina/cdp/manual` 控制 URL 读取策略。
- `--enable-cdp` 允许通过用户本地 `http://localhost:3456` CDP proxy 辅助读取少量 URL。项目只检测 proxy，不负责启动第三方工具；只操作自己创建的 tab，并在读取后关闭。
- `--enable-browser-history` 显式读取本地 Chrome/Edge 历史中的关键词匹配项。该能力默认关闭，只在用户本地运行，不保存完整历史，只保留匹配到的 `url`、`title`、`last_visit_time`。
- 多 URL 读取默认单线程；如需分治，可在 `config/web_access.yaml` 显式设置小的 `max_concurrency`，上限仍受保护以避免高频抓取。
...
- 不进行高频批量抓取，不默认使用 CDP。
- 对微信公众号等平台，建议优先使用用户提供的完整 URL；不要裁剪 query 参数。
- 静态请求失败不代表文章不存在，应使用手动正文、CSV 或合规数据源兜底。
- 本地浏览器历史检索只读取关键词匹配项，不保存完整历史。

## LLM 配置

LLM 由 `src/llm/factory.py` 统一构建，支持 `mock`、`gemini`、`qwen`、`deepseek`、`glm` 和自定义 `openai-compatible` provider。没有配置对应 key 时，默认回退到 `MockLLMClient`，保证 CLI、API、前端和测试流程可以跑通。
