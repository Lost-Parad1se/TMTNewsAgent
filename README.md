# TMTNewsAgent

TMTNewsAgent 是一个面向券商研究所互联网/TMT 行业的投研信息整理 Agent 原型。项目支持从公众号文章 URL、本地 CSV、可扩展搜索 API 中获取公开文章线索，完成正文清洗、去重、公司映射、主题分类、LLM 辅助摘要和投研简报生成。

本项目重点是合规、可维护、可扩展的投研工作流演示，不实现绕过登录、验证码、访问限制、反爬机制或高频批量抓取。

## 功能亮点

- 多源信息接入：手动 URL、本地 CSV、搜索 API adapter 骨架。
- 稳健网页读取：支持 static / Jina / 可选本地浏览器辅助 / 本地浏览器历史检索骨架。
- 公众号/公开网页正文解析：支持常见 WeChat HTML 字段，如 `rich_media_title`、`js_name`、`js_content`。
- 新闻去重与质量控制：URL、标题归一化和标题相似度去重。
- 互联网/TMT 公司和主题标签体系：内置重点公司别名和行业关键词。
- LLM 辅助摘要：支持 OpenAI-compatible API，缺少 key 时自动使用 MockLLM。
- 投研简报自动生成：输出 Markdown 与 JSON。
- SQLite 存储：保存原始文章、处理结果和简报。
- 失败兜底与日志追踪：单篇失败不会中断整个 pipeline。

## 合规说明

本项目仅用于公开信息整理和学习研究。生产环境应接入合法授权的数据源，不应绕过平台登录、验证码、访问限制或反爬机制。第三方 WeChat/公众号数据工具如 OpenClaw、Apify actor、公众号数据 API 等应作为可替换数据源，不作为强绑定依赖；使用前需确认授权边界、服务条款和数据合规要求。

## 架构流程

```text
Input Sources
  -> Collectors
  -> Extractors
  -> Cleaner
  -> Deduplicator
  -> Classifier / Entity Mapper
  -> LLM Summarizer
  -> Research Brief Writer
  -> SQLite / Markdown / JSON Outputs
```

## 目录结构

```text
TMTNewsAgent/
  config/                 # 数据源、主题、prompt 配置
  references/site_patterns/
                          # 站点访问模式、陷阱与合规边界
  data/raw/               # 手工维护 CSV 或输入样例
  data/processed/         # SQLite 数据库
  data/outputs/           # Markdown/JSON 简报与 pipeline_report
  src/
    collectors/           # manual_url / csv / search
    extractors/           # article extractor / html cleaner
    web_access/           # static / Jina / optional CDP / browser history
    processors/           # dedupe / classify / summarize / report
    llm/                  # replaceable LLM clients
    storage/              # SQLite and file stores
    utils/
  tests/
```

## 快速开始

安装依赖：

```bash
pip install -r requirements.txt
```

复制环境变量模板。没有 LLM key 也可以运行 mock 流程：

```bash
copy .env.example .env
```

运行 CSV demo：

```bash
python -m src.cli --mode csv --topic "AI算力与互联网平台动态" --csv-path data/raw/articles.csv
```

运行手动 URL 模式：

```bash
python -m src.cli --mode manual_url --topic "游戏出海动态" --urls "https://mp.weixin.qq.com/xxx" "https://mp.weixin.qq.com/yyy" --web-access-strategy auto
```

运行搜索骨架模式。未配置授权搜索 API 时会返回 mock 结果：

```bash
python -m src.cli --mode search --topic "大模型应用" --keywords "AI算力" "云计算" "腾讯控股" "阿里巴巴"
```

显式启用本地浏览器历史匹配模式：

```bash
python -m src.cli \
  --mode browser_history \
  --topic "AI算力近期动态" \
  --keywords "AI算力" "光模块" "英伟达" \
  --enable-browser-history \
  --since 7d
```

输出文件会写入 `data/outputs/`，包括：

- `brief_<date>_<topic>.md`
- `brief_<date>_<topic>.json`
- `pipeline_report.json`

SQLite 数据库默认写入：

```text
data/processed/tmt_news_agent.db
```

## FastAPI

启动 API：

```bash
uvicorn src.api:app --reload
```

启动后打开前端研究工作台：

```text
http://127.0.0.1:8000/
```

前端支持输入公司/关键词、时间窗口、手动 URL、授权搜索或本地浏览器历史匹配，并直接生成 Markdown 报告预览和 `.md` 下载文件。若要搜索 3 天内的微信公众号文章，可选择“授权搜索”，推荐配置 `TAVILY_API_KEY`，系统会聚焦查询 `site:mp.weixin.qq.com/s`；未配置授权搜索 API 时，请使用手动 URL、本地历史或 CSV/手动正文模式。

前端也可以切换基座模型 provider：`mock`、`gemini`、`qwen`、`deepseek`、`glm`、`kimi`、`openai-compatible`。API Key 仍然只从 `.env` 读取，前端不会要求输入或传输密钥。

请求示例：

```bash
curl -X POST "http://127.0.0.1:8000/run" ^
  -H "Content-Type: application/json" ^
  -d "{\"mode\":\"csv\",\"topic\":\"AI算力与互联网平台动态\",\"keywords\":[\"AI算力\",\"云计算\",\"腾讯\",\"阿里\"],\"urls\":[],\"csv_path\":\"data/raw/articles.csv\"}"
```

响应示例：

```json
{
  "status": "success",
  "brief_markdown_path": "...",
  "brief_json_path": "...",
  "pipeline_report": {
    "collected_count": 5,
    "extracted_count": 5,
    "deduplicated_count": 5,
    "summarized_count": 5,
    "fetch_strategy_stats": {},
    "cdp_used_count": 0,
    "manual_required_count": 0,
    "failed_fetch_items": [],
    "failed_items": [],
    "output_files": {}
  }
}
```

## 网页读取与本地辅助

`src/web_access/` 用于在 collector 和正文清洗之间提供更稳健、可选、低侵入的信息获取能力。设计参考 eze-is/web-access 的策略选择、site patterns、本地浏览器辅助和历史检索思路，但本项目只实现合规公开信息读取，不做绕过登录、验证码、反爬或平台访问限制。

默认策略：

- 普通文章页：优先 `jina`，再回退 `static`。
- 微信公众号 URL：优先 `static`；如果显式 `--enable-cdp`，才尝试本地 CDP；否则进入 `manual_required`。
- PDF URL：使用 `static`。
- 关键词搜索仍由 `SearchCollector` 处理，URL 抽取后再进入网页读取流程。

可选能力：

- `--web-access-strategy auto/static/jina/cdp/manual` 控制 URL 读取策略。
- `--enable-cdp` 允许通过用户本地 `http://localhost:3456` CDP proxy 辅助读取少量 URL。项目只检测 proxy，不负责启动第三方工具；只操作自己创建的 tab，并在读取后关闭。
- `--enable-browser-history` 显式读取本地 Chrome/Edge 历史中的关键词匹配项。该能力默认关闭，只在用户本地运行，不保存完整历史，只保留匹配到的 `url`、`title`、`last_visit_time`。
- 多 URL 读取默认单线程；如需分治，可在 `config/web_access.yaml` 显式设置小的 `max_concurrency`，上限仍受保护以避免高频抓取。

示例：

```bash
python -m src.cli \
  --mode manual_url \
  --topic "AI算力动态" \
  --urls "https://mp.weixin.qq.com/xxx" \
  --web-access-strategy auto
```

```bash
python -m src.cli \
  --mode manual_url \
  --topic "AI算力动态" \
  --urls "https://example.com/news/ai-compute" \
  --web-access-strategy jina
```

合规边界：

- 不绕过登录、验证码、反爬、付费墙或平台访问限制。
- 不进行高频批量抓取，不默认使用 CDP。
- 对微信公众号等平台，建议优先使用用户提供的完整 URL；不要裁剪 query 参数。
- 静态请求失败不代表文章不存在，应使用手动正文、CSV 或合规数据源兜底。
- 本地浏览器历史检索只读取关键词匹配项，不保存完整历史。

## LLM 配置

LLM 由 `src/llm/factory.py` 统一构建，支持 `mock`、`gemini`、`qwen`、`deepseek`、`glm`、`kimi` 和自定义 `openai-compatible` provider。没有配置对应 key 时，默认回退到 `MockLLMClient`，保证 CLI、API、前端和测试流程可以跑通。

Gemini：

```text
LLM_PROVIDER=gemini
GEMINI_API_KEY=...
GEMINI_MODEL=gemini-2.5-flash
```

Qwen / 阿里云百炼：

```text
LLM_PROVIDER=qwen
DASHSCOPE_API_KEY=...
QWEN_MODEL=qwen-plus
```

DeepSeek：

```text
LLM_PROVIDER=deepseek
DEEPSEEK_API_KEY=...
DEEPSEEK_MODEL=deepseek-v4-flash
```

GLM / 智谱：

```text
LLM_PROVIDER=glm
ZHIPUAI_API_KEY=...
GLM_MODEL=glm-5.1
```

Kimi / Moonshot：

```text
LLM_PROVIDER=kimi
MOONSHOT_API_KEY=...
KIMI_MODEL=moonshot-v1-8k
```

自定义 OpenAI-compatible 网关：

```text
LLM_PROVIDER=openai-compatible
LLM_BASE_URL=https://api.example.com/v1
LLM_API_KEY=...
LLM_MODEL=your-model
```

其他可选项：

```text
LLM_FALLBACK_TO_MOCK=true
LLM_TEMPERATURE=0.2
LLM_TIMEOUT_SECONDS=60
```

## CSV 输入格式

`data/raw/articles.csv` 支持以下字段：

```text
title,url,account_name,publish_time,raw_text
```

示例文件包含 5 条 mock TMT 新闻数据，均已标注为 mock，不伪造真实新闻出处。

## 测试

本项目测试兼容 `unittest`：

```bash
python -m unittest discover -s tests
```

如果已安装 pytest，也可以运行：

```bash
pytest
```

## 后续扩展方向

- 接入合规公众号数据 API、研报、公告、RSS 和新闻 API。
- 增加 RAG 知识库，用于公司历史事件和产业链背景补充。
- 增加前端看板，支持按公司、主题、重要性筛选。
- 增加定时任务和任务队列。
- 增加行业事件数据库和主题热度追踪。
- 增加公司画像、产业链图谱和可视化导出。
- 对接更多 LLM provider，并加入 prompt 版本管理和评估集。
